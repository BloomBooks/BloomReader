...
public class WebAppInterface {
    ...
    public void getCurrentPageMultimediaState() {
        mContext.runOnUiThread(new Runnable() {
            @Override
            public void run() {
                ...
                mWebView.evaluateJavascript("Root.requestPageMultimediaState()", new ValueCallback<String>() {
                    ...
                    public void onReceiveValue(String s) {
                        ...
                        // Since here we only expect a single value, we will just strip off the extra double quotes,
                        // but if someday we need a more generalized version of onReceiveValue,
                        // we'll need to use a JSON parser.
                        String strippedResult = s.length() < 3 ? "" :
                                s.substring(1, s.length() - 1);
                        mPageHasMultimedia = strippedResult.equals("true");
                    }
                });
            }
        });
