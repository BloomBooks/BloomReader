...
public abstract class BaseActivity extends AppCompatActivity {
    ...
    private void createFileObserver() {
        ...
        if (mHandler == null) {
            ...
            mostRecentlyModifiedBloomFileTime = getLatestModiedTime(new File(pathToWatch), mostRecent);
    ...
    protected void notifyIfNewFileChanges() {
        final String pathToWatch = BookCollection.getLocalBooksDirectory().getPath();
        String[] newMostRecent = new String[1]; // poor Java's out param
        long newModifyTime = getLatestModiedTime(new File(pathToWatch), newMostRecent);
        if (newModifyTime > mostRecentlyModifiedBloomFileTime) {
            mostRecentlyModifiedBloomFileTime = newModifyTime;
            onNewOrUpdatedBook(newMostRecent[0]);
        }
    }

    // lastModFile is a crude reference parameter. Pass an array of at least one
    // element and its first slot will be set to the last modified file's path.
    private static long getLatestModiedTime(File dir, String[] lastModFile) {
