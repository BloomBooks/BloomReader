...
public class ImportBundleTask extends AsyncTask<Uri, String, Void> {
    ...
    protected void onPostExecute(Void v) {
        ...
        if (importBundleErrorHandler.hasErrors()) {
            importBundleErrorHandler.toastError();
        }
    ...
    private class ImportBundleErrorHandler {
        ...
        private void toastError() {
            // We want errors toasted longer than the usual "I found a book" toast.
            mToast.setDuration(Toast.LENGTH_LONG);
            MainActivity activity = mainActivityRef.get();
            if (activity == null || activity.isFinishing())
                return;

            // Usually there will only be one... but be prepared. We will only do one toast,
            // but may need to handle more than one error.
            boolean fToasted = false;
            for(ImportErrorObject importError : mImportErrors) {
                reportSingleError(activity, importError, fToasted);
                fToasted = true;
            }
            // Resetting the toast to the default duration after toasting error.
            mToast.setDuration(Toast.LENGTH_SHORT);
        }

        private void reportSingleError(MainActivity activity, ImportErrorObject importError, boolean fToasted) {
            int toastMessageId;
            if (importError.mException == null) {
                ...
                toastMessageId = R.string.bundle_import_error;
            } else {
                ...
                toastMessageId = importError.mException.getMessage().contains("ENOSPC") ?
            ...
            if (!fToasted) {
                toast.setText(activity.getText(toastMessageId));
                toast.show();
            }
        }
