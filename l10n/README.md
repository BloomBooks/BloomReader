...
1. Download the translations. Either:
   - Run `downloadFromCrowdin.sh` (needs `BLOOM_CROWDIN_TOKEN`, `curl`, and Python 3; exits nonzero if any language failed). It exports the Bloom Reader
...
4. There will also be a bunch of new files added for languages we haven't included previously. Delete those
   (`git clean -fd -- 'app/src/main/res/values-*/'` removes just those folders).
